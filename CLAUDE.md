...
## Identity
- **Vessel**: Businesslog Ai
- **Role**: BusinessLog.ai
- **URL**: https://businesslog-ai.casey-digennaro.workers.dev
- **Repo**: github.com/Lucineer/businesslog-ai
- **Branch**: master
...
### Deploy
```bash
cd /tmp/businesslog-ai && wrangler deploy
```

### Health Check
```bash
curl -s https://businesslog-ai.casey-digennaro.workers.dev/health
curl -s https://businesslog-ai.casey-digennaro.workers.dev/vessel.json | python3 -m json.tool
```
...
| /api/admin/apikeys | API endpoint |
| /api/admin/audit | API endpoint |
| /api/admin/audit/user/:userId | API endpoint |
| /api/admin/backups | API endpoint |
| /api/admin/backups/:id/restore | API endpoint |
| /api/admin/retention | API endpoint |
| /api/admin/retention/enforce | API endpoint |
| /api/admin/users/invite | API endpoint |
| /api/analytics/dashboard | API endpoint |
| /api/analytics/report | API endpoint |
...
- **Vessel Tuner**: https://vessel-tuner.casey-digennaro.workers.dev/api/vessel?name=businesslog-ai
- **Fleet grid**: Listed in cocapn.ai and the-fleet

## Shipwright Mode — Drydock Operations

### Architecture Pattern
- **Type**: CF Worker (standard)
- **JSON helper**: Hono (app.get)
- **CSP pattern**: const CSP object
- **Features.js**: no

### Fleet Patterns
- **Frame-ancestors in CSP**: yes
- **vessel.json capabilities**: chat, strategy, finance
- **Fleet link footer**: yes

### Refactoring Rules
1. **NEVER** change the JSON helper function name (`Hono (app.get)`) — breaks all endpoints
...
1. Keep the vessel small. ~1551 lines is the current size.
